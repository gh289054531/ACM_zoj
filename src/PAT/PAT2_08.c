...
float op(int a[], char *op, int method)
{
	float tmp1;
	float tmp2;
	float tmp3;
...
int main()
{
	...
	for(j = 0; j < 24; j++){//four numbers have A44=4*3*2*1=24 sequences
		for(k = 0; k < 64; k++){//operator have 4*4*4=64 sequences
			for(l = 0; l < 5; l++){//op priority have 5 manners
				tmp = op(operand[j], operator[k], l);
				if(tmp == 24.0){
					switch (l){
						case 0:
